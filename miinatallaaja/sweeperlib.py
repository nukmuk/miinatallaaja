...
def create_window(width=800, height=600, bg_color=(240, 240, 240, 255)):
    ...
    if graphics["window"] is None:
        graphics["window"] = pyglet.window.Window(
        ...
        graphics["background"] = pyglet.sprite.Sprite(
            pyglet.image.SolidColorImagePattern(
                bg_color).create_image(width, height)
        )
...
def resize_window(width, height):
    ...
    graphics["background"] = pyglet.sprite.Sprite(
        pyglet.image.SolidColorImagePattern(graphics["bg_color"]).create_image(
            width, height
        )
    )
...
def prepare_sprite(key, x, y):
    ...
    graphics["sprites"].append(
        pyglet.sprite.Sprite(
            graphics["images"][str(key).lower()], x, y, batch=graphics["batch"]
        )
    )
